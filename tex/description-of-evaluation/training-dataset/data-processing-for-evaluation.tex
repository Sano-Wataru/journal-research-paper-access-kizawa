...
The activity levels were treated as a measure that integrates the subjective and objective perspectives.
  ...
  The labels were defined as the group labels (GL).
    %
    \deleted{GL represents the clustering results that capture multiple activity levels.}
    \revised{GL represented the clustering results that captured multiple activity levels.}
% k-meansの手順
...
Data extension compensated for the limited number of point-cloud datasets.
...
%3rd paragraph
% MLの定義
\deleted{Agreement and disagreement between the GL of the two datasets in each of the point-cloud datasets pairs are defined as matching labels (ML).}
  % MLの形式
  \deleted{ML is represented as a binary label.}
\revised{Agreement and disagreement between the GL of the two datasets in each of the point-cloud datasets pairs were defined as matching labels (ML).
  ML was represented as a binary label.}
Dataset pairs enabled supervised comparison of feature vector differences between sessions with known GL relationships.
